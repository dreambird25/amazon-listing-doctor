...
1. Establish the evidence lane, identity, product type, requirements, parentage level, locale, source, and timestamps. This step is complete when every unavailable scope field is explicit rather than silently defaulted.
...
3. Resolve the Skill directory containing this `SKILL.md`, resolve user files to absolute paths, change to the Skill directory, and run its deterministic engine:
   ...
   This step is complete when the report contains separate `current_listing_gate`, `candidate_preview_gate`, `release_decision`, `official_validation_completeness`, and `ptd_validation_coverage` fields. Treat `ptd_validation_coverage.mode=LIGHTWEIGHT_SUBSET` as an explicit limit, never as complete PTD Schema validation. Exit codes are `0` for no official/system error, `1` for an official error, and `2` for a system error.
...
- Only a fully bound `mode=VALIDATION_PREVIEW` response with status `VALID` can pass the same candidate payload. `ACCEPTED` is a real-submission response mismatch.
