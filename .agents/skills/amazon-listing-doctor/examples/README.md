...
`semantic-assessment.json` demonstrates all seven quality dimensions and the v1.6.0 `assessment_version=1.4` scope/content/report/manifest, locale, time, content-evidence scope, and Evidence Policy 1.2 binding. Its image rating binds a synthetic `visual_observation` recorded after viewing the synthetic image; URL and technical metadata alone would not satisfy that policy. It is bound specifically to the deterministic report generated from `listing-valid.json`; changing that input or report correctly makes the merge fail. Merge it with a freshly generated official report using `scripts/merge_report.py --output merged-report.json`. Generated reports are intentionally not committed because the scripts and tests are their source of truth.
