...
```json
{
  ...
  "current_content": {
    ...
    "images": [{
      ...
      "watermark": null,
      "visual_observation": "Optional objective description recorded after viewing the image"
    }]
  },
  "official": {
    "listing_snapshot": {
...
Listings Items attributes describe seller contribution and must use `content_scope=SELLER_CONTRIBUTION`; they are not proof of what a buyer currently sees. For `current_content.title`, an adapter may use the exact-Marketplace `summaries[].itemName` only when scoped `attributes.item_name` is absent. It must not cross Marketplaces or manufacture an `attributes.item_name` element. Storefront text captured from the scoped marketplace may use `BUYER_VISIBLE`. `OBSERVED_ABSENT` is meaningful only with a source whose complete response or observation contract proves that an omitted field is absent. If the adapter selects a subset, the API omits unrequested data, the page is partially loaded, or completeness is uncertain, use `PARTIAL|UNKNOWN` and `missing_field_semantics=UNKNOWN`. Invalid metadata is a system error. Omitted metadata defaults conservatively to unknown supplied content.
...
- `official_evidence_coverage`: separate completeness for current snapshot, candidate Preview, and local PTD validation. `current_listing_snapshot` and `candidate_preview` use `COMPLETE|INCOMPLETE`; `ptd_local_validation` uses only `FULL_JSON_SCHEMA|EVALUATED_SUBSET|INCOMPLETE`. Missing or unknown values remain unknown and are never inferred from aggregate gates.
...
- `quality_evidence_policy`: versioned, per-dimension minimum evidence result. Policy 1.2 records rule codes, content-module names, and `evidence_basis`, not raw product values. Missing-content claims require `OBSERVED_ABSENCE` plus complete source coverage. Image-content ratings require a manifest-bound `images[N].visual_observation`; URL and technical metadata alone are insufficient. A policy failure rejects the semantic merge.
- `executive_summary`: concise user-facing facts derived during a successful quality merge. Summary version 1.4 adds `evidence_stages` without changing canonical gates or the official report hash. Identity contains `marketplace_id + seller_sku + asin`; the ASIN remains optional context. `content_quality` owns the quality verdict, quality evidence completeness, score, reason, action, and `change_preview`. `change_preview.original_values` copies only evidence already bound to the selected quality dimension; `candidate_value` is present only when the selected recommendation produced a validated deterministic suggestion. `candidate_available=false` means only that no validated exact candidate was generated; the renderer must use a neutral message and must not infer missing facts or observation mode. `official_evidence` owns official validation completeness, source coverage, reason, and action. `evidence_stages` separately derives current snapshot acquisition, current snapshot issues, PTD local validation, candidate-content presence, candidate local validation, and candidate Preview. PTD keeps `FULL_JSON_SCHEMA` and `EVALUATED_SUBSET` distinguishable in the user-facing stage instead of treating lightweight validation as full validation. Compatibility aliases expose `quality_primary_*` and `official_primary_*`; a missing PTD or Preview must not erase a completed current issues snapshot, and a missing or untraceable official snapshot must not replace the content-quality reason. An applicable `OFFICIAL_ERROR` may remain the operational `primary_reason`, but both lanes are always preserved. The summary is additive and never replaces canonical official fields.

`scripts/render_report.py` defaults to `--view concise`, which renders a content-quality section, a compact original-value/candidate-value comparison table, and a separate Amazon official-evidence stage table. The display locale defaults to `zh-CN` when neither `report_locale` nor `--lang` is supplied; `--lang en` remains available. The content section shows localized evidence scope/coverage, score status, evaluated count, weak dimensions, structural comparability, quality reason, and quality action. A Seller-contribution scope is explicitly labeled as not equivalent to buyer-visible content. The official table separately shows current snapshot acquisition, current snapshot issues, PTD local validation, candidate content, candidate local validation, candidate Preview, release decision, and evidence completeness. A Preview that ran but is stale or otherwise unusable is not labeled as “not run.” The concise Markdown user view does not append stable English status/error codes or Amazon's original foreign-language message; if the assessment does not supply a usable Chinese business reason, the concise view uses a target-neutral localized reason while the detailed audit view retains the original text. `INCOMPLETE` official evidence means at least one separate stage is incomplete, not that Listing content or every official stage is incomplete. Use `--view detailed` for the concise conclusion plus stable codes, original messages, official findings, table-form seven-dimension results, table-form recommendations with original/candidate values, bound fact path/value/hash trace, limitations, and assessment trace. Both detailed Markdown and detailed JSON revalidate the embedded assessment and rederive the quality verdict and summary. Detailed JSON is idempotently revalidatable after display fields or `report_locale` are added. Invalid quality bindings are not rendered as trusted content; detailed JSON removes them and returns `quality_render_status=INVALID_ASSESSMENT`. “No finding” is not “passed” unless the corresponding official check completed successfully.
