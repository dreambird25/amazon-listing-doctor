...
- `official_report_sha256`: deterministic hash of the official report material before semantic quality fields are added. Copy it into the assessment's same-named binding field; `merge_report.py` recomputes and verifies it.
...
- `executive_summary`: concise user-facing facts derived during a successful quality merge. Identity contains `marketplace_id + seller_sku + asin`; the ASIN remains optional context. The summary copies official gates and completeness, exposes `evaluated_dimension_average`, primary reason/action, and `performance_verdict=NOT_EVALUATED`. The score exposes structural completeness and a comparison cohort hash; pairwise comparison requires both reports to be `FULL` and in the same cohort. The summary is additive and never replaces canonical official fields.

`scripts/render_report.py` defaults to `--view concise`, which shows Marketplace, Seller SKU, ASIN, canonical current/release decisions, official validation completeness, score status, evaluated count, weak dimensions, structural comparability, primary reason, and one action. Candidate gates are shown whenever they are not both `PASS` or release is not `PASS`. Use `--view detailed` for the concise conclusion plus official findings, all seven dimensions, recommendations, bound fact path/value/hash trace, limitations, and assessment trace. Both detailed Markdown and detailed JSON revalidate the embedded assessment and rederive the quality verdict and summary. Invalid quality bindings are not rendered as trusted content; detailed JSON removes them and returns `quality_render_status=INVALID_ASSESSMENT`. “No finding” is not “passed” unless the corresponding official check completed successfully.
