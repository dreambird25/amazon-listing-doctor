...
- `executive_summary`: concise user-facing facts derived during a successful quality merge. Identity contains `marketplace_id + seller_sku + asin`; the ASIN remains optional context. The summary copies official gates and completeness, exposes `evaluated_dimension_average`, primary reason/action, and `performance_verdict=NOT_EVALUATED`. It is additive and never replaces canonical official fields.

`scripts/render_report.py` defaults to `--view concise`, which shows Marketplace, Seller SKU, ASIN, canonical current/release decisions, official validation completeness, score status, evaluated count, weak dimensions, comparability, primary reason, and one action. Candidate gates are shown whenever they are not both `PASS` or release is not `PASS`. Use `--view detailed` for the concise conclusion plus official findings, all seven dimensions, recommendations, limitations, and assessment trace. “No finding” is not “passed” unless the corresponding official check completed successfully.
