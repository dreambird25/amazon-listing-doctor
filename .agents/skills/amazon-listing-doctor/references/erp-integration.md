...
- Use `--mode quality-observation` for unlabeled `input + assessment` practice runs. It aggregates quality behavior without turning model observations into Golden labels. Use explicit `--output` paths so Python writes multilingual artifacts as UTF-8 instead of relying on shell redirection.
