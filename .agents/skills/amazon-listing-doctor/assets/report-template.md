...
- 内容优化行动：一个可检查完成条件的行动
- 原始值 / 候选值：使用三列表格展示目标字段、已绑定原始值和候选值
- 候选值：仅在目标维度已评估，且输出可由已绑定原始标量值与标点/空格确定性生成时展示；否则中性显示“本轮未生成候选值”，不得猜测缺失原因

### Amazon 官方证据状态

- 使用紧凑表格分别展示：当前问题快照、当前快照问题、PTD 本地校验、候选内容、候选本地校验、候选官方预检、发布决策、官方证据完整性
- 机器审计视图继续保留稳定 gate；默认中文视图只显示业务标签
- 官方状态原因与行动：只描述 Amazon 官方问题或证据缺口
- `INCOMPLETE` 必须注明“至少一个独立校验环节尚未完成，请以上表为准”，不得覆盖已获取的当前快照，也不得写成 Listing 内容不完整

默认简洁结论不能隐藏官方 `BLOCK`、`UNKNOWN` 或 `INCOMPLETE`，也不能让普通官方证据缺口遮蔽内容质量原因。适用的 `OFFICIAL_ERROR` 必须在官方证据区明确展示并继续阻断操作。需要审计证据时，详细视图必须使用表格展示七维明细与建议，并包含简洁结论、官方 findings、原始值、候选值、局限和评估追溯。
