...
本地 PTD 引擎只执行当前支持的长度/数量约束，并通过 `ptd_validation_coverage` 明示 `LIGHTWEIGHT_SUBSET`；它不冒充完整 PTD Schema 校验。候选提交是否被 Amazon 接受仍以绑定同一 Payload 的 `VALIDATION_PREVIEW` 为准。
...
- `VALIDATION_PREVIEW` 只有在 mode、PUT/PATCH、身份范围和 Payload SHA-256 一致时才能通过相同候选。
