...
- `executive_summary` 升级到 1.4，新增只读派生的 `evidence_stages`；当前问题快照、快照问题、PTD 本地校验、候选内容、候选本地校验和候选 Preview 分开表达，不修改原有官方 gate、release decision 或报告哈希。
- 阶段派生保留已存在的候选 gate；即使兼容输入没有 `candidate.content`，已经执行的 Preview 仍按原 gate 展示，不会被降成“未执行”。完整 Schema 与轻量 PTD 校验也使用不同业务标签。
- 默认 Markdown 将 Amazon 官方证据改为高密度阶段表。已取得且无 issue 的当前快照不会再因为 PTD 或候选 Preview 未执行而显示成一条含糊的“当前 Listing 未评估”。
- 没有确定性候选时统一显示“本轮未生成候选值”，不再从 `candidate_available=false` 推断缺少产品事实、人工确认或观察模式。
- 中文简洁报告遇到外语质量理由时改用按维度/评级映射的中文业务理由；原始模型文本仍只保留在详细审计视图。
- 未显式提供 `report_locale` 或 `--lang` 时默认使用 `zh-CN`；中文理由映射采用当前/候选中性的“所评估内容”表述，并拒绝仅用少量汉字包裹的大段外语理由。
- 已执行但过期或不可绑定的候选 Preview 显示为“已提供，但当前不可用于判断”，不再误写成“未执行”。

### 数据归一化

- 明确 Listings Items 标题优先读取精确 scope 的 `attributes.item_name`；仅在缺失时回退同 Marketplace 的 `summaries[].itemName`，禁止跨站点取值或伪造 attributes。
- 私有 100 条 dev 观察中暴露的 4 条标题遗漏已用该规则修复并隔离复评；公共仓库只登记非识别性结论，不保存产品标识、正文或原始响应。

### 验证

- 公共合成测试增加到 139 个，覆盖证据阶段派生、快照与 PTD/Preview 分离、完整/轻量 PTD 状态、缺失阶段不反推、旧版候选兼容、无候选中性文案和中文简洁理由；私有适配器另以合成输入覆盖标题回退边界。
